...
var Proxies = fs
  .readFileSync("httpProxy.txt", "utf8")
  .split("\n")
  .filter(function (a) {
    return !!a;
  });
...
var spawnBuf = new Uint8Array([
  115, 10, 0, 42, 15, 32, 83, 112, 111, 111, 107, 121, 0, 66, 111, 116, 115,
]); 
...
Bot.prototype = {
  ...
  getValue: function (originX, originY, targetX, targetY) {
    ...
    // if (theta < 0) the ta += 360;     // [0, 360]; anticlockwise; 0° = east
  ...
  onOpen: function (b) {
    if (connectedCount > botCount) this.disconnect();
    client = this;
    ...
    if (2 <= lol.length) {
      if ("6" == f) {
        ...
        if (((lol[3] << 8) | lol[4]) == this.snakeID) {
          ...
          if (
            lol.length >= 31 &&
            (((lol[18] << 16) | (lol[19] << 8) | lol[20]) / 5.0 > 99 ||
              ((lol[21] << 16) | (lol[22] << 8) | lol[23]) / 5.0 > 99)
          ) {
...
function start() {
  for (var i in bots) bots[i].disconnect();
  ...
  for (var i in bots) bots[i].connect(); 
}   
